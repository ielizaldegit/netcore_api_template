...
public static class SpecificationBuilderExtensions
{
    ...
    public static ISpecificationBuilder<T> PaginateBy<T>(this ISpecificationBuilder<T> query, BaseFilter filter)
    {
        if (filter.Pagination.PageNumber <= 0)
        {
            filter.Pagination.PageNumber = 1;
        }
        ...
        if (filter.Pagination.PageNumber > 1)
        {
            query = query.Skip((filter.Pagination.PageNumber - 1) * filter.Pagination.PageSize);
        }
